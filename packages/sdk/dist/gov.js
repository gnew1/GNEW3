...
import GnewGovTokenAbi from "@gnew/contracts/artifacts/src/governance/GnewGovToken.sol/GnewGovToke;
n.json;
" assert { type: ";
json;
" }; ;
export function getGnewGovToken(address, signerOrProvider) {
    return new ethers.Contract(address, GnewGovTokenAbi.abi, signerOrProvider);
}
/packages/sdk / src / addresses.example.ts(añade, gov);
export const GNEW_GOV_TOKEN_ADDRESS = {
// 17000: "0x...", // Holesky 
...
ERC20;
de;
Gobernanza(coexistente, con, utility) - Contrato;
`src/governance/GnewGovToken.sol` - Roles;
`DEFAULT_ADMIN_ROLE`, `MINTER_ROLE`, `PAUSER_ROLE` - CAP;
de;
suministro(`MAX_SUPPLY`), faucet;
solo;
testnets(Goerli / Holesky / Amoy / Anvil) - Eventos;
`Minted`, `Burned`(+`Paused/Unpaused`, de, OZ) -  ** Opcional;
UUPS ** ;
`src/governance/GnewGovTokenUUPS.sol` + script `deployGovUUPS.ts`;
#;
#;
#;
Comandos `` `bash 
...
omite esto)* 
> Ya cubierto en `;
packages / contracts / hardhat.config.ts `. Se añadió 
`;
/hardhat-upgrades` para UUPS. ---;
#;
#;
Notas;
de;
...
N2 -  ** ERC20;
gobernanza;
inicial ** entregado(`GnewGovToken.sol`);
con `MINTER_ROLE`, `PAUSER_ROLE`, `Pausable`,  ** CAP ** . -  ** Eventos ** ;
`Minted`, `Burned`;
`Paused/Unpaused`;
provienen;
de;
...
mint(cap), pausas,
    reverts, faucet;
y;
cooldown.Configurado `forge snapshot`;
y `forge 
coverage`. -  ** Comandos ** listos;
`snapshot`, `coverage:forge`, despliegue / verify;
para;
gov;
y;
gov;
UUPS. -  ** Riesgos & Controles ** ;
recomendado;
uso;
de;
multi;
...
Events + The;
Graph(subgraph);
opcional.Entregables;
Emisión;
de;
Transfer / Mint / Burn + pruebas;
Hardhat / Foundry.Pasos;
Modelar;
eventos;
necesarios.Pruebas;
de;
invariantes(Echidna);
y;
unitarias.Pruebas / DoD;
100 % paths;
críticos;
property - based;
para;
saldos.Comandos;
echidna - test.Riesgos & Controles;
Falta;
de;
indización;
subgraph +
...
const toWei = (n) => ethers.parseUnits(n, 18);
describe("GnewGovToken — eventos (Hardhat)", () => {
    ...
    beforeEach(async () => {
        ...
        gov = (await F.deploy("GNEW-GOV", "gGNEW", owner.address, toWei("100"), toWei("1000"), toWei("5"), 3600));
    ...
    it("emite Paused/Unpaused y bloquea transferencias", async () => {
        await expect(gov.pause()).to.emit(gov, "Paused").withArgs(owner.address);
        await expect(gov.transfer(alice.address, 1)).to.be.revertedWith("paused");
        await expect(gov.unpause()).to.emit(gov, "Unpaused").withArgs(owner.address);
    });
});
...
describe("GnewToken — eventos (Hardhat)", () => {
    ...
    beforeEach(async () => {
        ...
        token = (await F.deploy("GNEW", "GNEW", owner.address, toWei("100")));
    ...
    it("emite Paused/Unpaused (OZ) y bloquea transferencias", async () => {
        ...
        await expect(token.transfer(alice.address, 1)).to.be.revertedWith("paused");
...
GovHandler;
{
    ...
    external;
    {
        ...
        admin;
        uint256;
        maxAdd = gov.MAX_SUPPLY() - gov.totalSupply();
        if (maxAdd == 0)
            return;
        uint256;
        amount = (amt % maxAdd) + 1;
    ...
    external;
    {
        ...
        uint256;
        bal = gov.balanceOf(address(this));
        if (bal == 0)
            return;
        uint256;
        amount = (amt % bal) + 1;
...
StdInvariant, Test;
{
    ...
    public;
    {
        gov = new GnewGovToken("GNEW-GOV", "gGNEW", address(this), 100e18, 1_000e18, 5e18, 10);
        handler = new GovHandler(gov);
        // asigna algo de balance al handler para fuzz 
        gov.mint(address(handler), 100e18);
        targetContract(address(handler));
    }
    function invariant_CapNotExceeded() { }
    public;
    {
        assertLe(gov.totalSupply(), gov.MAX_SUPPLY());
    }
    function invariant_NoTransfersWhenPaused() { }
    public;
    {
        if (gov.paused()) {
            ...
            balances;
            uint256;
            beforeBal = gov.balanceOf(address(handler));
            // uso try/catch para evitar revert 
            try { }
            finally { }
            handler.transfer(address(0xBEEF), 1, ether);
            { }
            try { }
            catch { }
...
GnewGovTokenEchidna;
{
    ...
    constructor();
    {
        ...
        gov = new GnewGovToken("GNEW-GOV", "gGNEW", address(this), 0, 1_000_000e18, 5e18, 1);
        // dotar fondos al harness para probar burns/transfers 
        gov.mint(address(this), 100e18);
    }
    // --- acciones fuzzables --- 
    function mint_as_admin(address, to, uint256, amt) { }
    public;
    {
        uint256;
        room = gov.MAX_SUPPLY() - gov.totalSupply();
        if (room == 0)
            return;
        uint256;
        amount = (amt % room) + 1;
    ...
    function try_transfer(address, to, uint256, amt) { }
    public;
    {
        uint256;
        bal = gov.balanceOf(address(this));
        ...
        (bool);
        ok, ;
        address(gov).call(abi.encodeWithSignature("transfer(address,uint256)", to, amount));
        // ok puede ser false en pausa; no afecta propiedades 
        ok;
    }
    function try_burn(uint256, amt) { }
    public;
    {
        uint256;
        bal = gov.balanceOf(address(this));
    ...
    // --- invariantes --- 
    function echidna_totalSupply_le_cap() { }
    view;
    returns(bool);
    {
        return gov.totalSupply() <= gov.MAX_SUPPLY();
    }
    function echidna_paused_blocks_transfers() { }
    returns(bool);
    {
        // fuerza pausa 
        if (!gov.paused())
            gov.pause();
        uint256;
        before = gov.balanceOf(address(this));
        (bool);
        ok, ;
        address(gov).call(abi.encodeWithSignature("transfer(address,uint256)", address(0xBEEF), 1));
    ...
    function echidna_burn_does_not_increase_supply(uint256, x) { }
    public;
    returns(bool);
    {
        uint256;
    ...
    / packages / contracts / package.json(añadir, scripts, Echidna, y, Foundry, invariants);
{
    ...
    {
        "clean";
        "rimraf artifacts cache out coverage typechain-types",
            "compile";
        "hardhat compile",
            "build";
        "hardhat compile && typechain --target ethers-v6 --out-dir typechain-types ;
        ...
        "forge snapshot",
            "echidna";
        "echidna-test ./echidna/GnewGovTokenEchidna.sol --contract GnewGovTokenEchidna --config ./echidna/echidna.config.yaml";
    }
    "devDependencies";
    {
...
Pruebas
    ** Eventos;
modelados ** -`Transfer(address from, address to, uint256 value)`(OZ) - `Minted(address to, uint256 amount, address by)` - `Burned(address from, uint256 amount, address by)` - `Paused(address account)` / `Unpaused(address account)`(OZ)
    ** Pruebas ** -Hardhat;
asserts;
de;
emisión;
...
eventos;
en;
mint / burn / pause(`test/events/*.spec.ts`) - Foundry;
invariantes(`test/invariants/GnewGovToken.Invariants.t.sol`) + unitarias;
ya;
existentes - Echidna;
property - based(`echidna/GnewGovTokenEchidna.sol`)
    ** Comandos **
        `` `bash 
pnpm --filter @gnew/contracts test:hh 
pnpm --filter @gnew/contracts test:forge 
