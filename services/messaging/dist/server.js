...
app.post("/prekeys", (req, res) => {
    ...
    if (!user || !identity_pk || !signed_prekey_pk)
        return res.status(400).json({ error: "missing" });
    db.prepare(`INSERT INTO prekeys(user, identity_pk, signed_prekey_pk, one_time_pk)
     ...
     ON CONFLICT(user) DO UPDATE SET
       ...
       one_time_pk=excluded.one_time_pk`).run({ user, identity_pk, signed_prekey_pk, one_time_pk: one_time_pk ?? null });
    res.json({ ok: true });
});
...
app.post("/messages", (req, res) => {
    ...
    db.prepare(`INSERT INTO messages(to_user, from_user, header, nonce, ciphertext)
     VALUES(?,?,?,?,?)`).run(to_user, from_user, JSON.stringify(header), nonce, ciphertext);
    res.json({ ok: true });
});
