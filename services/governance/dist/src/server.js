...
app.get("/participation/:proposalId", async (req, res) => {
    const governor = new ethers.Contract(process.env.GOVERNOR, // dirección governor
    [
    ...
    const token = new ethers.Contract(process.env.TOKEN, ["function getPastTotalSupply(uint256) view returns (uint256)"], governor.runner);
