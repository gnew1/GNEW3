...
export class PolicyWatcher {
    ...
    async current() {
        ...
        return { version: Number(version), uri, hash: hash, model, policy };
    }
    start(cb) {
        const handler = async (version, uri, hash) => {
            try {
                const { model, policy } = await this.fetchAndVerify(uri, hash);
    ...
    stop() { if (this.#timer)
        clearInterval(this.#timer); this.#contract.removeAllListeners(); }
}
