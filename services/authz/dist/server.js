...
import { PolicyWatcher } from "./chain/watcher";
...
let policyVersion = 0;
let enforcer;
const cache = new LRUCache({ max: 5000, ttl: 30_000 }); // 30s
...
app.post("/authz/evaluate", async (req, res) => {
    ...
    if (cached && cached.v === policyVersion) {
        ...
        const out = { allowed: cached.decision, policyVersion, decisionId, latencyMs: Math.round(latency) };
    ...
    const out = { allowed, policyVersion, decisionId, latencyMs: Math.round(latency) };
    return res.set("X-Decision-Id", decisionId).json(out);
});
