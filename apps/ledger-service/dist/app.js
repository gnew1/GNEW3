...
async function ensureBalanced(lines) {
...
app.post("/accounts", requireRole("ledger:write"), async (req, res) => {
    try {
        ...
        const r = await pool.query(`insert into gl_accounts(code,name,type,currency,metadata) values($1,$2,$3,$4,$5) returning *`, [body.code, body.name, body.type, body.currency, body.metadata ?? {}]);
...
app.post("/journal/entries", requireRole("ledger:write"), async (req, res) => {
    ...
    try {
        ...
        const r = await client.query(`insert into gl_entries(occurred_at, period_month, status, reference, external_ref)
       values($1,$2,'draft',$3,$4) returning *`, [body.occurredAt, period, body.reference ?? null, body.external_ref ?? null]);
        const entryId = r.rows[0].id;
        for (const ln of body.lines) {
            const accId = await getAccountIdByCode(client, ln.accountCode);
            await client.query(`insert into gl_entry_lines(entry_id, account_id, debit, credit, description, txid, chain, external_ref, metadata)
...
app.post("/journal/entries/:id/post", requireRole("ledger:write"), async (req, res) => {
    ...
    try {
       ...
       where l.entry_id=$1
       group by l.entry_id`, [id]);
        if (!sums.rowCount)
            throw new Error("entry_not_found");
        const { deb, cred, period } = sums.rows[0];
        if (Number(deb) !== Number(cred))
            throw new Error("unbalanced_entry");
        // check period lock
        const locked = await client.query("select locked from gl_periods where period_month=$1", [period]);
        if (locked.rowCount && locked.rows[0].locked === true)
            throw new Error("period_locked");
        const r = await client.query(`update gl_entries set status='posted', posted_at=now()
         where id=$1 and status='draft'
         returning id,status,posted_at`, [id]);
        if (!r.rowCount)
            throw new Error("cannot_post");
        await client.query("commit");
        res.json(r.rows[0]);
    }
    catch (e) {
...
app.post("/periods/:ym/lock", requireRole("ledger:admin"), async (req, res) => {
    ...
    const p = `${ym}-01`;
    await pool.query(`insert into gl_periods(period_month,locked,locked_by)
     values($1,true,$2)
     on conflict (period_month) do update set locked=true, locked_by=$2, locked_at=now()`, [p, req.user?.sub ?? "system"]);
...
app.get("/journal/entries/:id", requireRole("ledger:read"), async (req, res) => {
    const { id } = req.params;
    const r = await pool.query(`select e.*, json_agg(json_build_object(
...
app.post("/reconcile/auto", requireRole("ledger:write"), async (req, res) => {
    ...
    try {
        ...
        const ev = await client.query(`insert into ext_events(txid, chain, amount, external_ref, occurred_at)
       values($1,$2,$3,$4,$5) returning id`, [body.txid ?? null, body.chain ?? null, body.amount ?? null, body.external_ref ?? null, body.occurredAt ?? new Date().toISOString()]);
        ...
        if (match?.rowCount) {
            ...
            await client.query(`insert into gl_reconciliations(event_id, status) values($1,'unmatched')`, [evId]);
            await client.query("commit");
...
app.get("/export/xbrl", requireRole("ledger:read"), async (req, res) => {
    ...
    if (!/^\d{4}-\d{2}$/.test(ym))
        return res.status(400).json({ error: "bad_period" });
    const period = `${ym}-01`;
    const balances = await pool.query(`select a.code, a.name, a.type,
