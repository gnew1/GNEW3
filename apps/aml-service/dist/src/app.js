...
function authOptional(req, _res, next) {
    ...
    if (h?.startsWith("Bearer ") && JWT_PUBLIC_KEY) {
        try {
            ...
            req.user = { sub: String(dec.sub), roles: dec.roles, email: dec.email };
        }
        catch { /* ignore */ }
    }
    next();
...
app.post("/admin/model", requireRole("aml:admin"), async (req, res) => {
    ...
    await pool.query(`insert into aml_model(id, cfg) values('active', $1)
     on conflict (id) do update set cfg=$1, updated_at=now()`, [body]);
    res.json({ ok: true });
});
...
app.post("/admin/sanctions", requireRole("aml:admin"), async (req, res) => {
    ...
    try {
        ...
        for (const s of arr) {
            await client.query(`insert into sanctions(type,name,country,wallet,doc) values($1,$2,$3,$4,$5)
         on conflict (name,wallet) do update set type=excluded.type, country=excluded.country, doc=excluded.doc`, [s.type, s.name, s.country ?? null, s.wallet ?? null, s.doc ?? null]);
        }
...
app.post("/ingest/tx", requireRole("aml:ingest"), async (req, res) => {
    ...
    try {
        ...
        const { rows: vrows } = await client.query(`select count(1)::int as n
       from tx_events where user_id=$1 and ts > now() - interval '10 minutes'`, [body.userId]);
        const velocity = Number(vrows[0]?.n ?? 0);
        // Sanctions quick match
        const sMatch = await client.query(`select id,name,type,wallet from sanctions
       where (wallet is not null and (wallet=$1 or wallet=$2))
          or (lower(name)=lower($3) or lower(name)=lower($4)) limit 1`, [body.walletFrom ?? "", body.walletTo ?? "", body.userId, body.counterparty ?? ""]);
        ...
        // Persist tx event
        await client.query(`insert into tx_events(tx_id,user_id,counterparty,amount,currency,channel,ts,raw)
       values($1,$2,$3,$4,$5,$6,$7,$8)
       on conflict (tx_id) do nothing`, [body.txId, body.userId, body.counterparty ?? null, body.amount, body.currency, body.channel, nowIso, evidencePayload]);
        // Thresholds and mode
        const thresholdL2 = Number(model?.thresholdL2 ?? 0.9);
        const thresholdL1 = Number(model?.thresholdL1 ?? 0.75);
        const mode = (model?.mode ?? "shadow");
        let level = "none";
        if (sanctionHit || rules.escalateL2 || score >= thresholdL2)
            level = "L2";
        else if (rules.flag || score >= thresholdL1)
            level = "L1";
        ...
        let alertId = null;
        if (level !== "none") {
            const r = await client.query(`insert into aml_alerts(level,status,score,tx_id,evidence_id,explanations,rules)
         values($1,'open',$2,$3,$4,$5,$6) returning id`, [level, score, body.txId, evidence.id, explanations, rules]);
            alertId = r.rows[0].id;
        }
        await client.query("commit");
        res.status(201).json({
...
app.get("/alerts/:id", requireRole("aml:read"), async (req, res) => {
    const { id } = req.params;
    const r = await pool.query(`select a.*, e.hash as evidence_hash, e.prev_hash, e.payload
...
app.post("/alerts/:id/ack", requireRole("aml:analyst"), async (req, res) => {
    ...
    const r = await pool.query(`update aml_alerts set status=$2, updated_at=now() where id=$1 returning *`, [id, status]);
    if (!r.rowCount)
        return res.status(404).json({ error: "not_found" });
    res.json(r.rows[0]);
});
