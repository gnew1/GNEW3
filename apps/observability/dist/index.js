...
const env = z
    ...
    .parse(process.env);
const watched = env.CONTRACTS.split(",").map((pair) => {
    const [name, addr] = pair.split(":");
    const abi = ABIS[name]?.abi;
    if (!abi)
        throw new Error(`ABI no encontrada para ${name}`);
    return { name, address: addr, iface: new ethers.Interface(abi) };
});
...
const cTxFailed = new Counter({
    ...
    labelNames: ["contract", "source"], // source: "chain" | "webhook"
    registers: [registry]
});
...
async function loop() {
    try {
        ...
        for (let b = from; b <= to; b++) {
            ...
            if (txs.length === 0)
                continue;
            const txsToWatched = txs.filter((t) => typeof t?.to === "string" && watched.some((w) => w.address.toLowerCase() === String(t.to).toLowerCase()));
...
app.post("/webhooks/failure", async (req, res) => {
    try {
...
app.listen({ host: env.HOST, port: env.PORT }).then(() => {
    console.log(`[observability] ready on http://${env.HOST}:${env.PORT} — watching ${watched
