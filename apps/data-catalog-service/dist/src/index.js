 ...
 */
function abacCan(user, dataset, action) {
    if (user.roles?.includes("catalog:admin"))
        return true;
...
async function datahub(query, variables) {
...
async function searchDatasets(q, limit = 20) {
  ...
  `;
    const data = await datahub(query, {
        input: { type: "DATASET", query: q, start: 0, count: limit },
    });
    const results = data.search.searchResults.map((r) => {
        const e = r.entity;
        const owners = (e?.ownership?.owners ?? []).map((o) => o?.owner?.urn).filter(Boolean);
        const tags = (e?.globalTags?.tags ?? []).map((t) => t?.tag?.name).filter(Boolean);
...
function inferClassification(tags) {
    if (tags.includes("restricted") || tags.includes("pii") || tags.includes("secret"))
        return "restricted";
    if (tags.includes("internal"))
        return "internal";
    return "public";
}
async function getLineage(urn) {
    const query = `
  ...
  `;
    const data = await datahub(query, { urn });
    const up = data?.dataset?.upstreamLineage?.entities?.map((e) => e.urn) ?? [];
    const down = data?.dataset?.downstreamLineage?.entities?.map((e) => e.urn) ?? [];
    return { upstream: up, downstream: down };
}
async function getDataset(urn) {
    const query = `
...
async function upsertDataset(input) {
  ...
  `;
    ...
    return datahub(query, { mcp: mcps });
}
async function deleteDataset(urn) {
    const query = `
    mutation Delete($urn: String!) {
      deleteEntity(urn: $urn)
    }
  `;
    return datahub(query, { urn });
}
// ---------- App ----------
const app = express();
...
// Audit helper
function audit(user, event, meta) {
