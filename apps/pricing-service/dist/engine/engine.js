...
function matches(rule, input) {
    const s = rule.scope;
    if (s.skus && s.skus.length && !s.skus.includes(input.sku))
        return false;
    if (s.segments && s.segments.length && input.segment && !s.segments.includes(input.segment))
        return false;
    if (!inRange(input.riskScore, s.riskMin, s.riskMax))
        return false;
    if (!inRange(input.quantity, s.qtyMin, s.qtyMax))
        return false;
    return true;
}
function computeDiscount(price, d) {
    if (d.type === "fixed") {
