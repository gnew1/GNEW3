 ...
 */
function planWeeklyMissions(items, opts = {}) {
 ...
 */
export function registerMissionsRoutes(app) {
     ...
     *  }
     */
    app.post("/api/feedback/plan", (req, res) => handlePlanPost(req, res));
    /**
    ...
    app.get("/api/missions/weekly", (req, res) => {
        try {
            const { weekStart, seed } = req.query;
            let d;
            if (weekStart) {
                if (!ISO_DATE_RE.test(weekStart))
                    return badRequest(res, "weekStart debe tener formato YYYY-MM-DD");
                d = new Date(weekStart + "T00:00:00Z");
                if (Number.isNaN(+d))
                    return badRequest(res, "weekStart no es una fecha válida");
            }
            let s;
            if (seed !== undefined) {
...
function internalError(res, message, err) {
    let detail = '';
...
function validateFeedbackItem(it, index) {
...
function validateBaseObject(it, where) {
    if (!it || typeof it !== "object")
        return `${where} debe ser un objeto`;
    for (const key of ["id", "userId", "text", "timestamp"]) {
        if (!(key in it))
            return `${where}.${key} es requerido`;
    }
    return null;
}
function validatePrimaryFields(fields, where) {
    const { id, userId, text, timestamp } = fields;
    if (typeof text !== "string")
        return `${where}.text debe ser string`;
    if (text.length === 0)
        return `${where}.text no puede estar vacío`;
    if (text.length > MAX_TEXT_LEN)
        return `${where}.text excede el máximo de ${MAX_TEXT_LEN} caracteres`;
    if (typeof id !== "string" || id.length === 0)
        return `${where}.id debe ser string no vacío`;
    if (typeof userId !== "string" || userId.length === 0)
        return `${where}.userId debe ser string no vacío`;
    if (typeof timestamp !== "string" || Number.isNaN(+new Date(timestamp)))
        return `${where}.timestamp debe ser ISO válido`;
    return null;
}
function validateOptionalFields(fields, where) {
    const { tags, votes, sentiment } = fields;
    if (typeof votes !== "undefined" && typeof votes !== "number")
        return `${where}.votes debe ser number`;
    if (typeof sentiment !== "undefined" && typeof sentiment !== "number")
        return `${where}.sentiment debe ser number`;
    if (tags !== undefined && !Array.isArray(tags))
        return `${where}.tags debe ser array de strings`;
    return null;
}
...
function validateFeedbackArray(fb) {
    if (!Array.isArray(fb))
        return "Campo 'feedback' debe ser un array";
    if (fb.length === 0)
        return "Se requiere al menos un elemento de feedback";
    if (fb.length > MAX_FEEDBACK)
        return `Máximo permitido: ${MAX_FEEDBACK} elementos de feedback`;
    return null;
}
function parseOptions(options) {
    if (!options || typeof options !== "object")
        return { value: {} };
    const value = {};
    const min = options.minMissionsPerWeek;
    const max = options.maxMissionsPerWeek;
    const seed = options.seed;
    const week = options.weekStart;
    if (typeof min === "number")
        value.minMissionsPerWeek = clampInt(min, 1, 20);
    if (typeof max === "number")
        value.maxMissionsPerWeek = clampInt(max, 1, 30);
    if (typeof seed === "number")
        value.seed = Math.abs(Math.trunc(seed));
    const wk = parseWeekStartString(typeof week === "string" ? week : undefined);
    if (wk.error)
        return { value, error: wk.error };
    if (wk.value)
        value.weekStart = wk.value;
    return { value };
}
function parseWeekStartString(week) {
    if (!week)
        return {};
    if (!ISO_DATE_RE.test(week))
        return { error: "options.weekStart debe tener formato YYYY-MM-DD" };
    const d = new Date(week + "T00:00:00Z");
    if (Number.isNaN(+d))
        return { error: "options.weekStart no es una fecha válida" };
    return { value: d };
}
